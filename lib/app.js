...
// this is cool, but not super scalable. If you needed multiple
// servers, or dynos, to run your application because it became
// popular this type of thing would no longer work.
...
io
  ...
  .on('connection', (socket) => {
    ...
    // move your game logic outside of this file.
    // see the answer event
    ...
    socket.on('answer', async ({ answer, roundId, gameId, currentRoundNumber }) => {
      // move this logic outside of app.js
      const game = Game.findById(gameId)
      const result = await game.answer({ answer, userId: socket.user._id });
...
const scoreGame = async (gameId) => {
  ...
  const scores = rounds.reduce((acc, round) => {
    if (round.winner) {
      acc[round.winner._id] += round.score
    }

    if (round.drawerScore) {
      acc[round.drawer._id] += round.drawerScore
    }
