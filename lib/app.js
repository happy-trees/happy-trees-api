...
io
  ...
  // eslint-disable-next-line no-console
    ...
    socket.on('answer', async({ answer, roundId, gameId, userId }) => {
      console.log('answer', answer, roundId, userId);
      const round = await Round.findById(roundId);
      console.log('round', round);
      if(round.isLive && round.word === answer) {
        io.to(gameId).emit('correct answer', { isCorrect: true });
        Round.findByIdAndUpdate(roundId, { isLive: false, winnerId: userId, score: 3 });
      }
    });
