  ...
  // eslint-disable-next-line no-console
    ...
    socket.on('find game', async() => {
      ...
      if(waitingGames.length) {
        ...
        const user = await UsersByGame.findOneAndUpdate({ userId: gameUsers[0].userId }, { hasDrawn: true });
        console.log('updating has drawn', user);
        ...
        console.log('GAME CURRENT ROUND', game.currentRound);
...
const startIntermissionTimer = (gameId, currentRoundNumber) => {
  console.log('START INTERMISSION TIMER');
  let countdown = 10;
  let intervalId = setInterval(async() => {
    ...
    if(countdown === 0) {
      ...
      // const round = await Round.create({ gameId, drawerId: gameUsers[0].userId });
      // console.log('NEW ROUND currentRound round', game.currentRound, round);
