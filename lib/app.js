...
io
  ...
  // eslint-disable-next-line no-console
    ...
    socket.on('stroke', (data) => {
      ...
      // io.to(data.gameId).emit('stroke', data);
    });
    ...
    socket.on('find game', async() => {
      ...
      if(waitingGames.length) {
        ...
        const gameUsers = await UsersByGame.find({ gameId: waitingGames[0]._id });
        await Game.findByIdAndUpdate(waitingGames[0]._id, { isStarted: true });
        const startRound = await Round.create({ 
          gameId: waitingGames[0]._id, 
          drawerId: gameUsers[0].userId,
          word: 'trees'
        });
        io.to(waitingGames[0]._id).emit('start game', startRound);
      } else {
