  ...
  // eslint-disable-next-line no-console
    ...
    socket.on('answer', async({ answer, roundId, gameId, currentRoundNumber }) => {
      ...
      if(round.isLive && round.word === answer) {
        ...
        if(currentRoundNumber < 5) {
          startIntermissionTimer(gameId, currentRoundNumber, socket);
        }
    ...
    socket.on('find game', async() => {
      ...
      if(waitingGames.length) {
        ...
        startRoundTimer(game._id, startRound, socket);
...
const startRoundTimer = (gameId, round, socket) => {
  let countdown = 10;
  let roundTimerId = setInterval(async() => {
    ...
    if(countdown === 0) {
      ...
      if(round.roundNumber < 5) {
        startIntermissionTimer(gameId, round.roundNumber, socket);
      } else {
  ...
  socket.once('answer', async({ answer, roundId }) => {
    const round = await Round.findById(roundId);
    if(round.isLive && round.word === answer)
      clearInterval(roundTimerId);
  });
...
const startIntermissionTimer = (gameId, currentRoundNumber, socket) => {
  let countdown = 10;
  let intermissionTimerId = setInterval(async() => {
    ...
    if(countdown === 0) {
      ...
      startRoundTimer(gameId, round, socket);
    }
  }, 1000);
};
