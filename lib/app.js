  ...
  // eslint-disable-next-line no-console
    ...
    socket.on('answer', async({ answer, roundId, gameId, currentRoundNumber }) => {
      ...
      if(round.isLive && round.word === answer) {
        io.to(gameId).emit('correct answer', { isCorrect: true, answer, userId: socket.user._id });
        ...
        io.to(gameId).emit('wrong answer', { isCorrect: false, answer, userId: socket.user._id });
      }
    });
    ...
    socket.on('find game', async() => {
      ...
      if(waitingGames.length) {
        ...
        startRoundTimer(game._id, startRound);
...
const startRoundTimer = (gameId, round) => {
  let countdown = 10;
